...
const default_customer_review = defaultConfig["pii-redact"]["defaultReview"]
const default_sentiment_placeholder = defaultConfig["pii-redact"]["defaultPIIText"]
const documentConfig = defaultConfig["document-chat"]["config"]
var ws = null
var msgs = null
function PIIPage(props: AppPage) {
  ...
  return (
    <ContentLayout
      ...
      header={
        <Header
          variant="h1"
          description="Redact any PII data from your text"
          actions={<Button iconName="settings" variant="icon" onClick={() => setModalVisible(true)} />}
        >
          PII Identification<Link variant="primary" onClick={() => setModalVisible(true)}> ({selectedModelOption.label}) </Link>
        </Header>
      }
    >
      <Container fitHeight >
...
export default withAuthenticator(PIIPage)