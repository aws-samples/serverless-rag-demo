...
class BedrockLayerStack(NestedStack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        aws4auth_layer_arn = f"arn:aws:lambda:{region}:{account_id}:layer:{aws4auth_layer_name}:*"
        opensearchpy_layer_arn = f"arn:aws:lambda:{region}:{account_id}:layer:{boto3_bedrock_layer_name}:*"
        boto3_bedrock_layer_arn = f"arn:aws:lambda:{region}:{account_id}:layer:{opensearchpy_layer_name}:*"
        langchainpy_layer_arn = f"arn:aws:lambda:{region}:{account_id}:layer:{langchainpy_layer_name}:*"

        lambda_layer_policy = _iam.PolicyStatement(actions=[
        "lambda:PublishLayerVersion"
        ], resources=[aws4auth_layer_arn, opensearchpy_layer_arn, boto3_bedrock_layer_arn, langchainpy_layer_arn])
        containerize_build_job.add_to_role_policy(lambda_layer_policy)

        self.suppressor([containerize_build_job], 'AwsSolutions-IAM5', 'Will not fix: We cannot remove wildcard here, as the CodeBuild should have permissions to publish different layer versions')
