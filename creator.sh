...
then
    ...
    user_pool_id=$(aws cloudformation --region $deployment_region describe-stacks --stack-name ApiGwLlmsLambda"$infra_env"Stack --query 'Stacks[0].Outputs[?OutputKey==`user_pool_id`].OutputValue' --output text)
    client_id=$(aws cloudformation --region $deployment_region describe-stacks --stack-name ApiGwLlmsLambda"$infra_env"Stack --query 'Stacks[0].Outputs[?OutputKey==`client_id`].OutputValue' --output text)
    config_json=$(cat <<-END
    {  
    ...
    echo "$config_json" > artifacts/chat-ui/src/config.json
