...
then
    ...
    user_pool_id=$(aws cloudformation --region $deployment_region describe-stacks --stack-name ApiGwLlmsLambda"$infra_env"Stack --query 'Stacks[0].Outputs[?OutputKey==`user-pool-id`].OutputValue' --output text)
    client_id=$(aws cloudformation --region $deployment_region describe-stacks --stack-name ApiGwLlmsLambda"$infra_env"Stack --query 'Stacks[0].Outputs[?OutputKey==`client-id`].OutputValue' --output text)
